...
const initializePlot = async () => {
  ...
  Plot.registerNotificationFilter((batchId, notifications) => {
    console.debug(
      "Received notifications in the filter with batchId: " + batchId
    );
  ...
  Plot.registerGeotriggerHandler((batchId, geotriggers) => {
    console.debug(
      "Received geotriggers in the handler with batchId: " + batchId
    );
  ...
  try {
    if (Platform.OS === "ios") {
      ...
      if (
        ...
        console.debug(
          "Location permission denied!. Not initializing PlotProjects SDK."
        );
      }
    }
  } catch (err) {
